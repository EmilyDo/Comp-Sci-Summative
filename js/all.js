...
class CALENDAR {
    ...
    drawEvents() {  
        ...
        // Currently only support one event. Refactor if more than one events need to be displayed
        let eventMsg = this.eventList[calendar.active.formatted];
        if (eventMsg !== '') {
            eventTemplate += `<li>${eventMsg}</li>`
        } else {
            eventTemplate += "<li>There is no tutoring available today</li>";
        }
    ...
    drawDays() {
        ...
        days = days.map(day => {
            ...
            let date = new Date(`${Number(day.month) + 1}/${day.dayNumber}/${day.year}`);
            let formatted = this.getFormattedDate(date);

            if (date.getDay() == 1 || date.getDay() == 3) {
                this.eventList[formatted] = 'There is tutoring available from 3:30pm - 5:30pm at Earl of March Secondary School';
            } else {
                this.eventList[formatted] = '';
            }
